...
import { ScrollView, Text, Button, YStack, Paragraph, XStack, View } from "tamagui";
...
export default function LessonTab() {
  ...
  useEffect(() => {
    console.log("activeVoucher:", activeVoucher?.data);
    console.log("errorVoucher", error);
  ...
  return (
    ...
    <FlatList
      ...
      renderItem={({ item }) => {
        const voucherIsActive = Array.isArray(activeVoucher?.data)
          ? activeVoucher?.data?.some((v) => v.typeV === item.type)
          : false;
        return (
          <>
            <YStack
            ...
            >
              ...
              <XStack justifyContent="space-between">
                ...
                {!!!voucherIsActive?<Button icon={Lock} justifyContent="flex-start" gap={10} alignItems="center" iconSize={20} onPress={()=>{return}}>Content Locked</Button>:<Button
      ...
      ListEmptyComponent={
        isError ? (
              ...
              alignItems="center"
            >
              <Text>Data is Error, Try again later</Text>
            </YStack>
        ) : (
                      <YStack
              gap={8}
